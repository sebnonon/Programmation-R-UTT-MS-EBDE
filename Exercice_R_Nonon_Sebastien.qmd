...
```{r}
pairs(ozone[, sapply(ozone, is.numeric)])
```

On regarde la distribution de notre variable d'intérêt:

```{r}
library(ggplot2)
ggplot(ozone, aes(x = maxO3)) + 
  geom_histogram(fill = "blue", color = "white", bins = 20) +
  labs(title = "Distribution de la concentration en Ozone", x = "maxO3", y = "Fréquence") +
  theme_minimal()
```
